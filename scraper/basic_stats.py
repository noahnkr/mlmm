...
from utils import (
    BASIC_TEAM_STATS, ADVANCED_TEAM_STATS,
)
...
for _, matchup in matchups.iterrows():
    for team_col in ["team_a", "team_b"]:
        ...
        if key in seen: continue # We have already collected tihs teams stats
        seen.add(key)
        ...
        basic_stats = { "year": year, "team": team }
        ...
        print(basic_stats)
        basic_team_stats.append(basic_stats)

with open("data/basic_stats.csv", "w", newline="", encoding="utf-8") as f:
    writer = csv.DictWriter(f, fieldnames=basic_team_stats[0].keys())
    writer.writeheader()
    writer.writerows(basic_team_stats)